...
@section('breadcrumb')
<div class="col-sm-6">
    <h4 class="page-title text-left">Schedule</h4>
    <ol class="breadcrumb">
                                                ...
                                                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td>{{$shift->id}}</td>
                                                            <td>{{$shift->name}}</td>
                                                            <td>{{$shift->id==1?'- - -':$shift->start_time->format('h:i A') }}</td>
                                                            <td>{{$shift->id==1?'- - -':$shift->end_time->format('h:i A') }}</td>
                                                            <td>{{$shift->status?'Active':'Inactive'}}</td>
                                                            <td>
                                                                <a href="#edit{{$shift->id}}" data-toggle="modal" class="btn btn-success btn-sm edit btn-flat"><i class='fa fa-edit'></i> Edit</a>
                                                                <a href="#delete{{$shift->id}}" data-toggle="modal" class="btn btn-danger btn-sm delete btn-flat"><i class='fa fa-trash'></i> Delete</a>
                                                            </td>
                                                        </tr>
                                                        @endforeach
...
@foreach( $shifts as $shift)
@include('includes.edit_delete_shift')
@endforeach
